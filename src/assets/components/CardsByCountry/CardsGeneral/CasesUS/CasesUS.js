...
export default class FetchCasesUs extends React.Component {
  state = {
    cases: null,
    confirmed: null,
    recovered: null,
  ...
  async componentDidMount() {
    const url = 'https://covid19-brazil-api.now.sh/api/report/v1/us';
    ...
    return (
      <div 
        ...
        <p 
          ...
          <span 
            style={{color: '#00FF66'}}> {this.state.recovered} </span>
        </p>
        <p 
          className="mt-2">Óbitos confirmados: 
          <span 
            style={{color: '#FF0000'}}> {this.state.deaths}</span>
        </p>
      </div>
    );
  };
